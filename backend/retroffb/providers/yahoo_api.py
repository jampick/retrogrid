...
class YahooClient:
    ...
    RETROFFB_YAHOO_REPLAY=1) responses come from `raw_dir` only."""

    def __init__(self, auth: YahooAuth | None = None, raw_dir: Path = RAW_DIR, offline: bool | None = None) -> None:
        self.offline = os.environ.get("RETROFFB_YAHOO_REPLAY") == "1" if offline is None else offline
