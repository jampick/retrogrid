...
def _int(v: Any) -> int | None:
    try:
        return int(v)
    except (TypeError, ValueError):           # None, NaN, pd.NA
        return None
...
class NflversePlayerDirectory:
    def __init__(
        ...
        import pandas as pd                     # only the parquet path needs it; the shipped snapshot does not
    ...
    @classmethod
    def from_snapshot(cls, path: Path | str) -> NflversePlayerDirectory:
        """The directory as `snapshot()` froze it — what the prebuilt sim ships instead of parquet."""
        self = cls.__new__(cls)
        self._reset()
        for row in json.loads(Path(path).read_text()):
            *fields, rank = row
            self._add(Player(*fields), tuple(rank))
        return self

    def snapshot(self) -> list[list]:
        return [[p.id, p.name, p.short, p.position, p.team, p.number, p.headshot, list(self._rank[p.id])]
