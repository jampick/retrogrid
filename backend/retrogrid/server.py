"""RETRO//GRID console server: static frontend + one WebSocket per viewer."""
...
FLAGS = paths.DATA / "grammar_flags.json"
...
@app.get("/sprites/{name}.png")
def sprite(name: str):                                        # noqa: ANN201
    """Sprites you built win; the shipped sim's fill in behind them."""
    hit = paths.sprite(name) if name.replace("-", "").isalnum() else None
    return FileResponse(hit) if hit else JSONResponse({"detail": "Not Found"}, status_code=404)
