...
Picks the regular-season Sunday with the most games, then writes to data/slate/:
    ...
    .venv/bin/python scripts/build_slate.py [--week N]
"""
...
from retroffb.models import PlayRow  # noqa: E402
from retroffb.providers.slate import PLAYS_FILE, POOL_FILE, SLATE_FILE  # noqa: E402
from retroffb.providers.stub_league import YAHOO_HALF_PPR, pa_tier_key  # noqa: E402

SEASON = 2025
SRC = ROOT / "data" / "nflverse"
OUT = ROOT / "data" / "slate"
...
def build_pool(pbp: pd.DataFrame, slate: pd.DataFrame, week: int) -> list[dict[str, Any]]:
    from retroffb.providers.directory import TEAM_NAMES
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not pbp_path.exists():
        print("missing nflverse data — run scripts/fetch_nflverse.py first", file=sys.stderr)
        return 1
