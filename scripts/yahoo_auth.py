#!/usr/bin/env python
"""One-time Yahoo sign-in (docs/YAHOO.md). Stores data/yahoo/token.json; the
server refreshes it from then on.

  scripts/yahoo_auth.py            sign in, then pick the league
  scripts/yahoo_auth.py --league   re-pick the league with the stored token
  scripts/yahoo_auth.py --check    show what the adapter sees, change nothing
...
from retroffb.providers.yahoo import LEAGUE_FILE, list_leagues  # noqa: E402
from retroffb.providers.yahoo_api import AUTH_URL, YahooAuth, YahooClient, YahooError, credentials  # noqa: E402
...
def check(client: YahooClient, season: int, week: int) -> None:
    ...
    from retroffb.providers.yahoo import YahooLeagueProvider
